...
发布舱（DeployDesk）是一个 **纯 Rust** 桌面发布工具，用于管理 Web 前端项目的本地构建、静态产物上传、远程版本发布和一键回滚。当前实现使用 Slint 构建原生桌面界面，不再依赖 Node.js、Vue、Vite 或 Tauri。
...
- SSH/SFTP 部署：支持密码认证和私钥认证，通过 SFTP 上传构建产物。
